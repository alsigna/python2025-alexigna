...
from .rpc_redis import RPCRedis
...
class RPC:
    def __init__(
        ...
        self._redis = RPCRedis(redis_host, redis_port, redis_db, redis_password)
    ...
    def __exit__(
        ...
        self._redis.redis.close()
        return False

    def __getattr__(self, name: str) -> RPCMethod:
        queue = Queue(connection=self._redis.redis)
